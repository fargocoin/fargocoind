...
#if defined(NDEBUG)
# error "FargoCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "FargoCoin Signed Message:\n";
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    CAmount nSubsidy = (int64_t)(4e+8/Params().LastPOWBlock() * COIN);
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (block.vtx[0].GetValueOut() > blockReward && block.IsProofOfWork())
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (block.GetHash() != Params().GenesisBlock().GetHash())
    	nHeight = mapBlockIndex[block.hashPrevBlock]->nHeight;
    if (fCheckPOW && block.IsProofOfWork() && nHeight > Params().LastPOWBlock())
        return state.DoS(100, error("AcceptBlock() : reject proof-of-work at height %d", nHeight), 0, "late-pow");
    ...
    if (fCheckMerkleRoot) {
        ...
        if (block.hashMerkleRoot != hashMerkleRoot2)
            return state.DoS(100, error("CheckBlock(): hashMerkleRoot mismatch b: %s calc: %s \nblock:%s",
...
static bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex=NULL)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (mi == mapBlockIndex.end())
        {
        	LogPrintf("not found: %s \n", block.hashPrevBlock.GetHex());
            return state.DoS(10, error("%s: prev block not found", __func__), 0, "bad-prevblk");
        }

        pindexPrev = (*mi).second;

        if (pindexPrev->nHeight < Params().LastPOWBlock())
        {
...
bool VerifySignature(const CCoins& txFrom, const CTransaction& txTo, unsigned int nIn, bool fValidatePayToScriptHash, int nHashType)
{
    assert(nIn < txTo.vin.size());
    const CTxIn& txin = txTo.vin[nIn];
    const CTxOut& txout = pcoinsTip->GetOutputFor(txin);

    ScriptError serror = SCRIPT_ERR_OK;
    return VerifyScript(txin.scriptSig, txout.scriptPubKey, STANDARD_SCRIPT_VERIFY_FLAGS,
    		TransactionSignatureChecker(&txTo, nIn), &serror);
}

bool SignBlock(CWallet& keystore, CBlock &block, int64_t nFees) {
	...
	CKey key;
	CMutableTransaction txFinCoinStake;
	txFinCoinStake.nTime = GetAdjustedTime();
	CTransaction txCoinStake(txFinCoinStake);

	int64_t nSearchTime = txCoinStake.nTime; // search to current time

		int64_t nSearchInterval = 1;
		if (keystore.CreateCoinStake(keystore, block.nBits, nSearchInterval, nFees, txCoinStake, key))
		{
			if (txCoinStake.nTime >= chainActive.Tip()->GetBlockTime()+1)
			{
				// make sure coinstake would meet timestamp protocol
				//    as it would be the same as the block timestamp
				block.vtx[0].nTime = block.nTime = txCoinStake.nTime;
				CMutableTransaction fstTx(block.vtx[0]);
				block.vtx[0] = CTransaction(fstTx);
