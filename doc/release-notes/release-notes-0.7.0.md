FargoCoin version 0.7.0 is now available for download at:
  http://sourceforge.net/projects/bitcoin/files/FargoCoin/bitcoin-0.7.0/

We recommend that everybody running prior versions of bitcoind/FargoCoin-Qt
...
in your terminal, then install the fargocoin-qt package:
  sudo apt-get install fargocoin-qt
...
Code:
/Applications/FargoCoin-Qt
(on Mac) or
Code:
bitcoind/fargocoin-qt
(on Linux).
...
FargoCoin Improvement Proposals implemented
...
* (Windows only): enable ASLR and DEP for fargocoin-qt.exe
* (Windows only): add meta-data to fargocoin-qt.exe (e.g. description)
